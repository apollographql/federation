...
For this tutorial, you'll use a graph key for the supergraph you'll build.
...
2. Enter a **Graph title**, for example `Quickstart Demo`. Leave the **Graph Architecture** set to `Supergraph (Default)` and click **Next**.
3. Next, a **Publish your Schema** dialog appears. Copy the protected value that appears after `APOLLO_KEY=` in the example code block. It begins with `service:`. This is your graph API key.
4. Copy the value that appears after `rover subgraph publish` and save it to a local text file. This is your graph ref and you'll use it in step 3. Click **Finish later**.
5. Provide your API key to Rover by running the `config auth` command:

    ```shell showLineNumbers=false
    rover config auth
    ```

6. Paste your personal API key into the prompt when requested.
...
Try running this sample query by copying and pasting it into the **Operation** window:
...
Once you run the operation, you should see this response in the right window:

```JSON showLineNumbers=false
to-do
```

You can try other operations to learn more about the available data.

Next, you'll publish your subgraph schemas to Studio to make your supergraph externally accessible and start recording metrics.
...
2. In your terminal, replace the final value `<GRAPH_REF>` with the graph ref you saved in [step 1](#authenticate-rover).
...
You'll see these in action by running a sample query for getting a user's cart given the user's ID.

Run this query by copying and pasting it into the **Operation** window:
