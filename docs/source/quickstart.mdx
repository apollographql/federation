...
The retail demo supergraph comes with `supergraph-config-dev.yaml` and `supergraph-config-dev.yaml` YAML configuration files that includes subgraph names, URLs, and router configurations for the retail demo supergraph.
