...
sidebar_title: "1️⃣ Project setup"
...
* On our local machine with the Rover CLI (we'll do this in [Part 3](./quickstart-pt-3/))
...
> ⚠️ If you already have Rover globally installed and you installed it with `npm`, first _uninstall_ that version with `npm uninstall`. This is required for the Federation 2 alpha.
...
> ⚠️ To support the Federation 2 alpha, you need to install `@apollo/gateway@latest-2` specifically, _not_ `@apollo/gateway`.
...
Our gateway project is ready, we've installed Rover, and we have a Studio account! Next, we'll start [composing our supergraph schema in Studio.](./quickstart-pt-2)
