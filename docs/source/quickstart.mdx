...
At the end of this quickstart, you will have set up a federated GraphQL API that integrates data from product, user, and order services using a retail demo supergraph.
Along the way, you'll learn about Apollo toolks like the Rover CLI and GraphOS Studio Explorer.
...
To do so, use the `rover dev` command with the `--supergraph-config` option.
