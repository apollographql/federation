...
By the end of this quickstart, you'll have set up a federated GraphQL API that integrates data from product, user, and inventory services using a demo retail supergraph.
Along the way, you'll learn about Apollo tools like the Rover CLI and GraphOS Studio.
...
- A local version of Apollo's demo retail supergraph
...
Once you've published your subgraphs, you can use the GraphOS Studio to explore and better understand your supergraph.
...
To start local development, you must first get the subgraphs in the [demo retail supergraph](https://github.com/apollosolutions/retail-supergraph) up and running.
...
The demo retail supergraph comes with `supergraph-config-dev.yaml` and `router-config-dev.yaml` YAML configuration files that include subgraph names, URLs, and router configurations.
...
- Rover uses Apollo Federation to compose a supergraph schema by adding subgraph schemas one at a time.
  ...
  - The order in which Rover adds subgraphs Rover adds is undefined, so you might see composition errors during intermediate steps.
...
3. Confirm that once you run the operation, you see the following response in the right window:
  ...
  This operation requests a list of all products from the `products` service.
  Next, you'll execute a query that demonstrates the power of federation.
...
Congrats! You've used one query to get information from across different services, including the `users`, `products`, and `inventory` subgraphs.
...
In the last part of this step, you'll learn more about how the router accomplished this.
...
You can toggle to view the query plan instead of the response by clicking **Response** and selecting **Query Plan**.
...
1. Take the example code block you copied from Studio and replace the last three lines with the following:
    ...
    - The `--routing-url` is the address the router should send requests to. Ensure your subgraphs are still [running locally](#run-subgraphs).
    
    <Note>

    In a production supergraph, subgraph routing URLs are the public endpoints for each service.
...
With these subgraphs published, you can explore and interact with their schemas in GraphOS Studio.

<Note>

The demo retail supergraph contains other subgraphs—for example, `reviews` and `shipping` subgraphs.
You can publish these to your supergraph if you want to explore their schemas and data.

</Note>

## Explore your supergraph

Once you've published your schemas, when you open the **Schema** page from the left nav in [Studio](https://studio.apollographql.com/?referrer=docs-content), you'll see all the subgraphs you've published.
...
GraphOS offers many other tools for understanding and managing your schema, including schema proposals, checks, and linting. GraphOS also includes schema delivery tools to integrate schema publication into your DevOps workflows.

Once you deploy your router, either by self-hosting it using the Apollo-managed cloud option, you can use it to collect and export telemetry to GraphOS Studio and your application performance monitoring (APM) and observability tools.
...
- Learn about the different [router types](/graphos/routing).
