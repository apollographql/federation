...
To learn more about federation concepts and the benefits of using federation, see the [Introduction to Apollo Federation](./).
...
1. Schema design
2. Registering subgraphs in GraphOS Studio
...
## Step 1. Update subgraph schemas

When building a supergraph, it can be helpful to use a schema-first approach.
With the schema in place, the frontend team can being build the client side app while backend developers develop and deploy backend services.

This tutorial uses two subgraphs:

- A `Products` subgraph with information about products in a retail business
- A `Reviews` subgraph with reviews for those products

It's standard retail practice to shows product details alongside reviews that users have given a product.
You can imagine that before federation, a client might need to make two separate requests to retrieve both sets of data.
With a federated graph, all of the data can be retrieved with just one call.

Imagine the `Product` type has a schema like this:

```yaml title="Products subgraph"
type Product {
  id: ID!
  name: String!
  price: Float!
...
The `Reviews` subgraph may have information about `Product` types as well:


```yaml title="Reviews subgraph"
type Product {
  id: ID!
  overallRating: Float
  reviewsForProduct: [Review]!
}
...
So that a client can request a `Product`'s details such as description and image URL along with review data such as a product's overall rating and individual reviews, you need to update both schemas.

Specifically, you need to denote `Product` as an entity—an object type that can resolve its fields across multiple subgraphs.
You do this by adding a `@key` to both schemas:
...
The `@key` directive defines an entity's _unique key_, which consists of one or more of the type's `fields`. See the [entity guide](./entities) to learn more.

Backend service owners also need to write reference resolvers for these types. See the [entity guide example](./entities#2-define-a-reference-resolver) for a subgraph using the Apollo Server library.

## Step 2. Register subgraphs
...
3. In the dialog that appears, specify an **Organization** and **Graph title** for your graph. Leave the **Graph Architecture** as **Supergraph (Default)**. Click **Next**. 

5. Leave the **Supergraph Pipeline Track** dropdown set to its default value. Click **Finish later**.

## Step 3: Configure your router

In a supergraph, a router acts as an entry point to your subgraphs and provides a unified interface for clients to interact with.
...
With GraphOS, you can choose for Apollo to provision and manage a cloud-hosted router for you or you can self-host your router.
This quickstart walks you through setting up your own self-hosted Apollo Router.

1. On your development machine, first create a new directory for your router project. Then inside that directory, run the following to install the Apollo Router:
...
If you do, you'll get a startup error message like `⚠️  The Apollo Router requires a composed supergraph schema at startup. ⚠️`

That's because your aren't currently providing a supergraph schema to the router. We'll fix that soon.
