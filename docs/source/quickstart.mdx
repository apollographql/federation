...
You've now installed Rover and ready to start local development with your supergraph!
...
To make your supergraph externally accessible, you individually publish each subgraph schema to GraphOS. 
...
2. Enter a **Graph title**, for example `Federation Quickstart`. Leave the **Graph Architecture** as `Supergraph (Default)`, and **Visible to Team Members** as `On` and click **Next**.
3. A **Publish your schema using Federation** dialog appears. Copy the example codeblock to a local text file.
...
4. Leave the window open, you'll complete setup with Rover.
...
1. Take the example codeblock you copied from Studio and replace the last three lines with the following:
...
Nice! Now if you open the **Schema** page from the left nav in [Studio](https://studio.apollographql.com/?referrer=docs-content), you'll see all the subgraphs you've published.
...
For the sample query in the final step, only the subgraphs in the instructions are needed, but you can publish the others to your supergraph if you want to explore their schemas and data.

### Step 4. Query your supergraph

Once you've published subgraph and GraphOS has successfully composed your supergraph schema, you can execute queries against your supergraph. To do that, you'll use the GraphOS Studio Explorer.
...
Running operations on a published supergraph from the Explorer has the additional benefit of collecting and visualizing metrics on the operations you run.
...
2. Open the **Settings** tab and click **Edit** next to **Connection settings**. Enter `http://localhost:4000/` as you supergraph's **Endpoint**.
...
The previous sample operation is a powerful one because it gathers data across various subgraphs: `users`, `products`, `inventory`, and more.
The GraphOS Studio Explorer displays _query plans_ in the right panel to help you understand how the router  intelligently orchestrate subrequests to these subgraphs. 
