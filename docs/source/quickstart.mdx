...
sidebar_title: "1️⃣ Local schema composition"
...
The supergraph's schema is the combination of each subgraph's schema, plus some special federation-specific directives. Each subgraph's schema can even reference and extend types that are also defined in _another_ subgraph.

This architecture enables clients to query data from multiple subgraphs simultaneously, just by querying the gateway.
...
> During the Federation 2 alpha, your installed versions of Rover and its Federation 2 plugin must be _identical_. If you encounter an error when using the plugin, try updating _both_ binaries to their latest version.
...
> 📣 **Federation 2 changes:** To support the Federation 2 alpha, you need to install `@apollo/gateway@latest-2`, not just `@apollo/gateway`.
...
As a best practice, your gateway should _not_ be responsible for composing its own supergraph schema. Instead, a separate process composes the schema and provides it to the gateway. This helps improve reliability and reduce downtime when you make changes to a subgraph.
