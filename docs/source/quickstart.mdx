...
3. A **Publish your schema using Federation** dialog appears. Copy the protected value that appears after `APOLLO_KEY=` in the example code block. It begins with `service:`. This is your _graph API key_.
...
You've now authenticated Rover with GraphOS and are ready to start local development with your supergraph!
...
4. Do the same thing for the `inventory` subgraph, substituting your graph ref:
...
5. Do the same thing for the `users` subgraph, substituting your graph ref:
...
Nice! Now if you open your graph in [Studio](https://studio.apollographql.com/?referrer=docs-content) now, you'll see all the subgraphs you've published on the **Schema** page.
  ...
  - The **SDL** tab displays your schema's raw SDL, filterable by subgraph
...
The GraphOS Studio Explorer dynamically displays example _query plans_ for your operations in the right-hand panel.
