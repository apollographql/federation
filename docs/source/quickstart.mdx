...
At the end of this quickstart, you will have set up a federated GraphQL API that integrates data from product, user, and inventory services using a demo retail supergraph.
...
The demo retail supergraph comes with `supergraph-config-dev.yaml` and `router-config-dev.yaml` YAML configuration files that includes subgraph names, URLs, and router configurations.
...
3. Confirm that once you run the operation, you see the following response in the right window:
  ...
  This operation requests a list of all products from the `products` service. Next you'll try a query that shows the power of federation.


4. Copy and paste the following query into the **Operation** window but don't run it yet.
...
Congrats! You've queried your router and gotten information from across different services.
...
In the last step of this tutorial, you'll publish your locally running subgraph schemas to GraphOS Studio.
...
4. Do the same for the `orders` subgraph, substituting your graph ref:
...
You can publish the others to your supergraph if you want to explore their schemas and data.
