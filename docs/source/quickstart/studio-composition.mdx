...
To compose our supergraph schema with Studio, we use a feature called  **managed federation**. With managed federation, each of our subgraphs publishes its schema to Apollo. Whenever a subgraph schema changes, Apollo composes a new supergraph schema.
...
Meanwhile, our router regularly polls Uplink for changes to its supergraph schema:
