...
Hello! This tutorial gets you up and running with Apollo Federation 2.
...
Usually, each subgraph corresponds to a different service in your backend. The supergraph is then represented by a **graph router**, which routes each incoming query to the appropriate combination of subgraphs and returns the combined result.
...
>
> Apollo Server can _also_ act as your graph router, as documented in [The graph router](../gateway/). This tutorial uses the Apollo Router because it's recommended for all new supergraphs (for details, see [Choosing a router library](../gateway/#choosing-a-router-library)).
...
Our router directory is ready, we've installed Rover, and we have a Studio account! Next, we'll start [composing our supergraph schema in Studio.](./studio-composition/)
