...
[Back in Part 2](./studio-composition/), we used managed federation with Apollo Studio to compose our router's supergraph schema. Next, let's try out composing locally with the Rover CLI.
