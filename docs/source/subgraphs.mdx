...
As part of our federated architecture, we want _other_ subgraphs to be able to extend the `User` type this subgraph defines. To enable this, we add the `@entity` and `@key` directives to the `User` type's definition to designate it as an [entity](./entities/):
