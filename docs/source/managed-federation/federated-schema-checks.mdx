...
For an introduction to schema checks, see [Schema checks](/graphos/schema-development/pipeline/checks).
...
When you run `rover subgraph check` on a particular subgraph, GraphOS Studio takes your schema changes and attempts to compose a supergraph schema from all of your registered subgraph schemas.
...
In most cases, you should run `rover subgraph publish` only after a successful run of `rover subgraph check`. However, certain workflows require intentionally publishing a subgraph schema that fails composition (such as [migrating an entity or field between subgraphs](/graphos/schema-development/federated-schemas/entities/migrate-fields)).
