...
* [Schema checks](https://www.apollographql.com/docs/studio/schema-checks/) (paid)
...
> Individual services do _not_ push trace data directly to Studio.

Traces are displayed in Studio in the shape of the query plan, with each unique fetch to a federated service reporting timing and error data.
...
To enable federated tracing, you [set the `APOLLO_KEY` environment variable](https://www.apollographql.com/docs/apollo-server/monitoring/metrics/#connecting-to-apollo-studio) in your _gateway's_ environment. **Do not** set this environment variable for your subgraph servers.

> If _other_ features require you to set `APOLLO_KEY` in your subgraph servers, [disable usage reporting](https://www.apollographql.com/docs/apollo-server/api/plugin/usage-reporting/#disabling-the-plugin) in those servers.
