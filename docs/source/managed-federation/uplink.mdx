...
By default in versions of `@apollo/gateway` vTODO and later, your gateway polls the GCP URL at a regular interval. Whenever a poll request fails, the gateway retries the request with the AWS URL. It continues swapping between the two URLs until the request succeeds, or until reaching the maximum number of retries.

> Verisons of `@apollo/gateway` prior to vTODO don't support multiple Uplink endpoints and only use the GCP URL by default.
...
For example, this configuration polls the AWS URL exclusively:

```bash
APOLLO_SCHEMA_CONFIG_DELIVERY_ENDPOINT=https://aws.uplink.api.apollographql.com/
```

### Retry limit

You can configure how many times your gateway retries a failed poll request like so:
...
By default, the gateway retries a number of times equal to three times the number of [Uplink URLs](#uplink-urls).
