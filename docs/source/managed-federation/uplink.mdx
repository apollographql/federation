...
If your supergraph schema _does_ exceed 6MB, you can set up a [build status webhook](/graphos/metrics/notifications/build-status-notification) for your graph. Whenever you're notified of a successful supergraph schema composition, your webhook can fetch the latest supergraph schema via the Rover CLI.

## Legacy endpoint (deprecated)

Instead of using Uplink, `@apollo/gateway` versions 0.33 and earlier poll a Google Cloud Storage URL to obtain their subgraph schemas and then run composition locally. **This polling method is deprecated and might be removed in the future.**

If you're using an earlier version of `@apollo/gateway`, we strongly recommend updating to the latest version to poll Uplink instead.
