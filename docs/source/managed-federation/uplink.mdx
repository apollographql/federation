...
const gateway = new ApolloGateway({
  experimental_pollInterval: 15000 // 15 seconds
});
```

The `experimental_pollInterval` option specifies the polling interval in milliseconds. This value must be at least `10000` (which is also the default value).
