...
* Your subgraphs publish their schemas to GraphOS, which stores them in its schema registry. GraphOS then [validates](/graphos/schema-development/pipeline/checks) and [composes](/graphos/schema-development/federated-schemas/composition) them into a supergraph schema. 

* Your routers can poll GraphOS&mdash;specifically, its [Apollo Uplink](/graphos/router-configuration/uplink) endpoint&mdash;to get the latest validated supergraph schema and other configurations.
...
Ready to try it out? Continue to [Setup](/graphos/get-started/quickstart).
