...
* Your subgraphs publish their schemas to GraphOS, which stores them in its schema registry. GraphOS then [validates](/graphos/schema-development/pipeline/checks) and [composes](/graphos/schema-design/federated-schemas/composition) them into a supergraph schema. 
