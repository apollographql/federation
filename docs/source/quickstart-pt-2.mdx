...
Meanwhile, our gateway regularly polls Apollo for changes to its supergraph schema:
...
> For the next step, you'll need the name of the graph you just created in Apollo Studio. Make sure to have it available.
...
Now, open up GraphQL Playground in your browser again at `localhost:4000`. Sure enough, your supergraph schema's types and fields are all available, and you can execute test queries as before. But now, all of this information is coming from Apollo Studio.
...
If we were now to register changes to one of our subgraph schemas, our running gateway would automatically fetch the corresponding changes to the supergraph schema. However, because we're currently using Apollo-hosted services that we can't modify, it doesn't make sense to register changes to our subgraph schemas.
