...
This API reference documents the exports from the `@apollo/gateway` package. This package enables you to use Apollo Server as a gateway for a federated supergraph. For more information, see [Node.js gateway setup](../building-supergraphs/router/#nodejs-gateway-setup).

> We recommend that all supergraphs use the Apollo Router instead of this Node.js-based gateway. See [Choosing a router library](../building-supergraphs/router/#choosing-a-router-library).
