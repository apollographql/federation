...
`string | SupergraphSdlHook | SupergraphSdlManager`
</td>
<td>
...
When `supergraphSdl` is a `SupergraphSdlManager`: An object containing an `initialize` property. `initialize` is an `async` function of the `SupergraphSdlHook` type mentioned directly above.
