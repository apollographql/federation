...
`IntrospectAndCompose` will use the data sources constructed by `ApolloGateway`. In order to customize the gateway's data sources, you can specify a [`buildService`](#buildservice) function.
...
Specify this option to enable supergraph updates via subgraph polling. `IntrospectAndCompose` will poll each subgraph
</td>
</tr>
