...
An object to use for logging in place of `console`. If provided, this object must implement all methods of [the `Logger` interface](https://github.com/apollographql/apollo-server/blob/main/packages/apollo-server-types/src/index.ts#L187-L193).
...
The drop-in replacement for `serviceList`. You pass an instance of this class directly to `ApolloGateway`'s `supergraphSdl` constructor option.
