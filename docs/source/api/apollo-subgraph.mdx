...
> This method previously existed in the `@apollo/federation` package (now removed by federation 2) and was renamed from `buildFederatedSchema` after `@apollo/federation` v0.28.0 (the previous name still works, but it might be removed in a future release).
