...
- [Define advanced keys](/graphos/schema-development/federated-schemas/entities/define-keys)
...
In Apollo Federation 2.3 and later, you can also apply `@key` to `interface` definitions to create [entity interfaces](/graphos/schema-development/federated-schemas/entities/interfaces/). If you apply `@key` to an `interface` in earlier versions of Federation 2, a composition error occurs.
...
See also [Advanced `@key`s](/graphos/schema-development/federated-schemas/entities/define-keys).
...
Most commonly, you set this to `false` when [referencing an entity without contributing fields](/graphos/schema-development/federated-schemas/entities/contribute-fields#referencing-an-entity-without-contributing-fields).
...
[Learn more about entity interfaces.](/graphos/schema-development/federated-schemas/entities/interfaces/)
...
See also [Value types in Apollo Federation](/graphos/schema-development/federated-schemas/sharing-types/) and [Resolving another subgraph's field](/graphos/schema-development/federated-schemas/entities/resolve-another-subgraphs-fields).
...
Indicates that a definition in the subgraph schema should be omitted from the router's [API schema](/graphos/schema-development/federated-schemas/schema-types/#api-schema), even if that definition is also present in other subgraphs. This means that the field is not exposed to clients at all.

Common use cases for `@inaccessible` include:

- Avoiding composition errors while making staggered updates to a definition that's shared across multiple subgraphs (such as a [value type](/graphos/schema-development/federated-schemas/sharing-types/#adding-new-shared-fields))
...
For more information, see [Using `@inaccessible`](/graphos/schema-development/federated-schemas/sharing-types/#using-inaccessible).
...
You can apply `@override` to [entity](/graphos/schema-development/federated-schemas/entities/) fields and fields of the root operation types (such as `Query` and `Mutation`).
...
For more information, see [Migrating entity and root fields](/graphos/schema-development/federated-schemas/entities/migrate-fields).
...
To learn more, see the [Incremental migration with `@override`](/graphos/schema-development/federated-schemas/entities/migrate-fields#incremental-migration-with-progressive-override) guide.
...
- `percent(<percent-value>)` - The percentage of traffic for the field that's resolved by this subgraph. The remaining percentage is resolved by the other ([from](#from)) subgraph. To learn more, see [Incremental migration with `@override`](/graphos/schema-development/federated-schemas/entities/migrate-fields#incremental-migration-with-progressive-override).
...
For more information, see [Using `@provides`](/graphos/schema-development/federated-schemas/entities/resolve-another-subgraphs-fields#using-provides).
...
See also [Contributing computed entity fields](/graphos/schema-development/federated-schemas/entities/contribute-fields#contributing-computed-entity-fields).
...
- All fields referenced in the `ContextFieldValue` must be expressed within the current subgraph. If the fields are referenced across multiple subgraphs, they must be annotated with [`@external`](/graphos/schema-development/federated-schemas/entities/use-contexts#referencing-fields-across-subgraphs) .
...
For examples using `@context` and `@fromContext`, see [Using contexts to share data along type hierarchies](/graphos/schema-development/federated-schemas/entities/use-contexts).
