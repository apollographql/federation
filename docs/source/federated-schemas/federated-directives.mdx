...
See also [Advanced `@key`s](../../entities/define-advanced-keys).
...
See also [Value types in Apollo Federation](./sharing-types/) and [Resolving another subgraph's field](../entities/resolve-another-subgraphs-fields).
...
To learn more, see the [Incremental migration with `@override`](../entities/migrate-fields#incremental-migration-with-progressive-override) guide.
...
- `percent(<percent-value>)` - The percentage of traffic for the field that's resolved by this subgraph. The remaining percentage is resolved by the other ([from](#from)) subgraph. To learn more, see [Incremental migration with `@override`](../entities/migrate-fields#incremental-migration-with-progressive-override).
...
For more information, see [Using `@provides`](../entities/resolve-another-subgraphs-fields#using-provides).
...
See also [Contributing computed entity fields](../entities/contribute-fields#contributing-computed-entity-fields).
...
- All fields referenced in the `ContextFieldValue` must be expressed within the current subgraph. If the fields are referenced across multiple subgraphs, they must be annotated with [`@external`](../entities/use-contexts#referencing-fields-across-subgraphs) .
...
The `@cost` directive defines a custom weight for a schema location. This customizes the operation cost calculation of the Enterprise Router [demand control feature](/router/executing-operations/demand-control/).

If `@cost` is not specified for a field, the default value is used. Scalars and enums default to a cost of 0, while composite input and output types default to a cost of 1.

Regardless of whether `@cost` is specified on a field, the type cost for that field also accounts for its arguments and selections.
...
The `@listSize` directive is used to customize the cost calculation of the Enterprise Router [demand control feature](/router/executing-operations/demand-control/).

In the static analysis phase, the cost calculator does not know how many entities will be returned by each list field in a given query. By configuring a field with `@listSize`, the cost calculator can more accurately estimate the cost during static analysis.
...
In this case, all queries for `items` are expected to receive at most 10 items in the list.
...
In this example, the `items` field can be requested with paging parameters. If the client sends a query with multiple slicing arguments, the scoring algorithm will use the maximum value of all specified slicing arguments. The following query is assumed to return 10 items in the scoring algorithm.
...
Sending the the above `MultipleSlicingArgumentsQuery` to a graph with this updated schema would result in an error, as would sending a query with no slicing arguments.
...
If both this and `assumedSize` are specified, `assumedSize` will take precedence.
