...
- The header `Apollo-Expose-Query-Plan` must be set with one of the following options:
  ...
  - A value of `dry-run` will generate the query plan and then abort exectution. This can be helpful if you want to warm up any internal or external [query plan caches](https://www.apollographql.com/docs/graphos/routing/performance/caching/in-memory)
