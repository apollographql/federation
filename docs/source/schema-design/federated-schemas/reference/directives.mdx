...
Apollo-provided directives like `@key` and `@connect` offer built-in functionality, whereas `url` values in a custom `@link` serve as labels for namespacing and versioning. Composition has no built-in knowledge of custom specs, so the `directive @myDirective(a: String!) on FIELD_DEFINITION` and `directive @hello on FIELD_DEFINITION` definitions shown above are required.
