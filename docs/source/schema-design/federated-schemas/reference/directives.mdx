...
Enum types are supported in the `format` argument. If you encounter composition errors when using enums, use Apollo Router v2.8.2 or later, which includes a fix for enum validation.

</Note>

**Programmatic tags:** To set cache tags programmatically (when tags don't depend on root field arguments or entity keys), add `apolloEntityCacheTags` or `apolloCacheTags` to your subgraph response extensions. For details, see [Programmatic cache tags](/graphos/routing/performance/caching/response-caching/invalidation#by-cache-tag).
