...
For more information on how to apply cache invalidation with cache tags, see the documentation on [response cache invalidation](/graphos/routing/performance/caching/response-caching/invalidation#by-cache-tag).
