...
This is a first Rust Native Apollo composition release. This release does not contain any new features. Rust composition
generates semantically equivalent supergraphs as the previous JavaScript binaries.

#### Additional Validations

##### Disallow Interfaces Implementing `@interfaceObject`

This pattern is not currently supported by the federation. Composition now generates an `INTERFACE_OBJECT_USAGE_ERROR`
error when it detects such scenario and fails the composition.
...
Composition now rejects custom specs imports that specify Apollo spec domain `https://specs.apollo.dev`. Previously,
custom spec URLs were only checked against individual Apollo specs. This was problematic as new Apollo specs could start
colliding with previously defined user specs.

##### Input Object Validation

Composition now validates user provided default object values to ensure they are valid (i.e. all fields are optional
or have default values). Invalid values are removed from the supergraph.

##### `@tag` Validations

`@tag` validations are now run as part of the main composition process (previously they were only run while contract
variants).

##### Root Type Inference Fix

Fixed GraphQL spec validation to ensure that composition will only infer default root operation types (e.g. `Mutation`)
if they are not referenced by other schema elements.

##### Stricter FieldSet Coercion Rules

`_FieldSet/FieldSet` is a custom scalar that represents a GraphQL selection set (minus brackets). While we expect this
value to be a `String`, additional values were accepted due to auto coercion logic. Composition will no longer accept
list of strings as a valid field set value.

#### Cosmetic Changes

##### Additional Hints

Merge process now correctly emits hints on composition failure.
...
Whenever subgraph defines a default value with coercible value (e.g. default value of `Int` for field that accepts `Float`),
this value will now be coerced to the appropriate target type (e.g. `weight: Float = 1` becomes `weight: Float = 1.0`).

##### Example Queries

Due to the differences in the underlying data structures and libraries, new composition version may include different
