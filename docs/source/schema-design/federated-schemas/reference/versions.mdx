...
This is the first Rust Native Apollo composition release. This release does not contain any new features. Rust composition
...
Federation does not currently support this pattern. Composition now generates an `INTERFACE_OBJECT_USAGE_ERROR`
error when it detects such a scenario and fails the composition.
...
Composition now rejects custom spec imports that specify the Apollo spec domain `https://specs.apollo.dev`. Previously,
Custom spec URLs are now checked against the entire Apollo spec domain to prevent future collisions with new Apollo specs.
colliding with previously defined specs in your schema.
...
Composition runs `@tag` validations as part of the main process (previously they ran while contract variants).
variants).

##### Root type inference fix

Composition ensures that it only infers default root operation types (e.g. `Mutation`)
...
While we expect this value to be a `String`, the system accepted additional values due to auto coercion logic.
