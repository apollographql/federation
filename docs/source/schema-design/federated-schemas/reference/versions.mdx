...
URLs are now checked against the entire Apollo spec domain to prevent future collisions with new Apollo specs.
...
Composition now runs `@tag` validations as part of the main process (previously they very checked during contract variant
generation only).

##### Root type inference fix

Composition ensures that it only infers default root operation types (e.g. `Mutation`) if they are not referenced by
