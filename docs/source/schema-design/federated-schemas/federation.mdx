...
Apollo Federation enables you to declaratively combine multiple APIs into a single federated supergraph. A client makes a single request to this supergraph, and it orchestrates calls to multiple APIs to produce a single response. 

A client makes a request to a supergraph via the federated GraphQL API's single entry point called the _router_. The router intelligently orchestrates and distributes the request across your APIs and returns a unified response. For a client, the request and response cycle of querying the router looks the same as querying any GraphQL server.
...
To jump into building a federated GraphQL with existing REST APIs, check out the [Apollo Connectors Quickstart](/graphos/get-started/guides/rest-quickstart).
