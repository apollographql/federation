...
**Q: What happens if I don't mark a field as @shareable in the source of truth?**

A: Composition fails with an error. You must mark the field as `@shareable` in at least one subgraph (the source of truth) for `@provides` to work.

**Q: How can I tell if `@provides` improves performance?**
