...
- The entity field must be marked as `@shareable` in at least one other subgraph. This ensures at least one subgraph can always resolve the field.

For more information, see [Best Practices for Using the @provides Directive](/graphos/schema-design/federated-schemas/entities/provides-directive).