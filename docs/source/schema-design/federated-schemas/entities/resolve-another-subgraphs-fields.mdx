...
For comprehensive guidance on when and how to use `@provides`, including best practices, troubleshooting, and detailed examples, see [Best Practices for Using the `@provides` Directive](/graphos/schema-design/federated-schemas/entities/provides-directive).