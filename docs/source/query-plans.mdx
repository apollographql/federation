...
flowchart TB;
  ...
  subFetchHotels --> collect("Collect (latestReviews,[],hotel)")
```
...
> In query plan diagrams shown by the Apollo Studio Explorer, `Flatten` nodes are called `Collect` nodes instead.
...
The `Flatten` node's `path` argument tells the gateway _at what position_ to merge the newly returned data with the existing data. An `@` element in a `path` indicates that the _previous_ path element returns a list.
...
* The first fragment is a **representation** of the entity being resolved (in this case, `Hotel`). [Learn more about entity representations](/entities/#resolving).
...
* [In Apollo Workbench](./workbench/overview/#view-query-plans), the VS Code extension for designing federated graphs
