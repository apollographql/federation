...
A query plan is defined as a hierarchy of **nodes** that resembles a JSON or GraphQL document.
...
As an example, we can return to the `GetHotels` example from our [example graph](#example-graph):
...
The `Flatten` node's `path` argument tells the gateway _at what position_ to merge the newly returned data with the existing data.

> An `@` element in a `path` indicates that the _previous_ path element returns a list.
