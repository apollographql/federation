...
In the Apollo Router v0.16.0+ and Apollo Gateway v2.5.4+, you can pass the following headers to return the query plans in the GraphQL response extensions:

* Including the `Apollo-Query-Plan-Experimental` header returns the query plan in the response extensions
* Additonally, including `Apollo-Query-Plan-Experimental-Format` with one of the supported options changes the output format:
