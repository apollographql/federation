...
| [`Defer`](#defer-node) | Tells the router about one or more `@defer` blocks at the same nested level. It contains a primary block and an array of deffered blocks. |
| [`Condition`](#condition-node) | Tells the router to split a query plan into two possible paths that can change at runtime. |
...
### Defer node
A `Defer` node corresponds to one or more `@defer` at the same level of "nestedness" in the planned query.

It contains a "primary block" and an array of "deferred blocks". The "primary block represents the part of the query that is not deferred (so the part of the query up until we reach the @defer(s) this handle), while each "deferred block" correspond to the deferred part of one of the `@defer` handle by the node.

Read more about how `@defer` operatates at [Apollo Router support for @defer](/router/executing-operations/defer-support/).

### Condition node
A `Condition` node is used to split a plan into an if-else branch. This is commonly used for `@skip` and `@include` to have a query plan that can select different nodes based on the provided runtime variables.
