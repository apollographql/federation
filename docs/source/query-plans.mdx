...
In the latest versions of Apollo Router and Gateway you can pass the following headers to return the query plans in the response extensions:

* Sending `Apollo-Query-Plan-Experimental` will return the query plan in the response extensions
* Sending `Apollo-Query-Plan-Experimental-Format` with one of the supported options will change the format:
  * A value of `prettified` will return the human readable formatted string  
  * A value of `internal` will return the JSON representation
