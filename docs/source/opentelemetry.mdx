...
> Apollo Studio does not currently consume OpenTelemetry-formatted data. To push trace data to Studio, see [Federated trace data](./metrics/).
>
...
const zipkinExporter = new ZipkinExporter({
  // url: set_thisifnot_running_zipkin_locally
});
provider.addSpanProcessor(
...
> Reminder: Federated gateways must not install the `@opentelemetry/instrumentation-graphql` library, so these spans are not included in its traces.
