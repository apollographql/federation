...
To use OpenTelemetry in your application, we recommend installing _at least_ the following `@opentelemetry` Node.js libraries (you can install additional instrumentation libraries as desired):
...
For a complete example of a federated graph that enables OpenTelemetry for the gateway _and_ each subgraph, [see this GitHub repo](https://github.com/apollographql/apollo-server-opentelemetry).

## GraphQL-specific spans

The `@opentelemetry/instrumentation-graphql` library emits the following spans as part of [OpenTelemetry traces](https://opentelemetry.io/docs/concepts/data-sources/#traces):
