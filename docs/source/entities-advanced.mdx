...
Compound keys can also include _nested_ fields. In the following example, the `User` entity's unique key consists of both a user's `id` _and_ the `id` of that user's associated `Organization`:
