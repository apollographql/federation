...
3. Update your router's supergraph schema to migrate to the updated Billing subgraph, either gradually or all at once.

    * **To migrate gradually**, use the [progressive `@override` feature](/router/configuration):

      1. Configure your router to enable progressive `@override`:

          ```yaml title="router.yaml"
          progressive_override:
            enabled: true
          ```
      1. In the Billing subgraph, add a `labels` argument to the `@override` directive with the percentage of requests that you want to be resolved by the subgraph. For example:

        ```graphql title="Billing subgraph"
        type Bill @key(fields: "id") {
          id: ID!
          amount: Int! @override(from: "Payments", labels:"percent(15)") #highlight-line
        }
        ``` 

        Setting `labels:"percent(15)"` means that 15 percent of the requests for `amount` are resolved by the Billing subgraph, while the remaining 85 percent are resolved by the Payments subgraph. 
