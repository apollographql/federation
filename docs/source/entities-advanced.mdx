...
When resolving a reference for an entity with multiple keys, you can determine how to resolve based on which key is present:
  ...
  Product: {
    __resolveReference(productRepresentation) {
      if(productRepresentation.sku){
        return fetchProductBySku(productRepresentation.id);
      } else {
