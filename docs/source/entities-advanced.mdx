...
3. Update your router's supergraph schema to migrate to the updated Billing subgraph.
   ...
   * Use progressive `@override` by adding a `label` argument to the `@override` directive with the percentage of requests that you want to be resolved by the subgraph. For example:
...
4. Gradually increase the percent of traffic directed to the Billing subgraph until the migration is completed by setting `label: "percent(100)"`.
