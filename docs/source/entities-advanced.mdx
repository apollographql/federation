...
It means that either `id` or (`id` _and_ `sku`) are enough to uniquely identify the entity. And since only `id` is enough, the query planner always uses only that field to resolve the entity.
