...
3. Update your router's supergraph schema to migrate to the updated Billing subgraph.
    ...
    * **To migrate gradually with progressive `@override`**, you must use the Apollo Router with an Enterprise license. We recommend using progressive `@override` if it's available.
       ...
       To use progressive `@override`, add a `label` argument to the `@override` directive with the percentage of requests that you want to be resolved by the subgraph. For example:
