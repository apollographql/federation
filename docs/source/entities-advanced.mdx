...
### Using `__typename` as `@key`

In some cases it may be useful to define an entity as a [singleton](https://en.wikipedia.org/wiki/Singleton_pattern). The `@key` directive supports the `__typename` [Meta Field](https://graphql.org/learn/queries/#meta-fields) to declare and entity as a singleton.
