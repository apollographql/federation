...
Although nested fields are most commonly used in compound keys, you can also use a nested field as a single `@key` field.
For example, the previous `User` entity `@key` doesn't make much sense without including the user `id`:
