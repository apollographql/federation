...
You can customize the `@key` of an entity to best identify the entity across subgraphs: 

- Create a [compound `@key`](#compound-keys) with multiple fields.
- Define [multiple `@key`s](#multiple-keys) for an entity. 
- Assign [different `@key`s across subgraphs](#differing-key-across-subgraphs) for an entity.
