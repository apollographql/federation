...
A subgraph that [references an entity](./entities/#referencing-an-entity) without extending it can include the fields of any `@key` in its stub definition:
...
2. Publish the updated Billing subgraph schema to Apollo.
    ...
    * Note that you're publishing a composition error, because multiple subgraphs define the same fields of `Bill` without marking them [`@shareable`](#using-shareable). This creates a failed [launch](https://www.apollographql.com/docs/studio/launches/) in Apollo Studio. That's okay! Apollo Uplink continues to serve the most recent _valid_ supergraph schema to your gateway.
...
6. Deploy the updated Payments subgraph to your environment.
    ...
    * This resolves the composition error, because all `Bill` fields are now resolved by a single subgraph. Apollo composes an updated supergraph schema, which your gateway automatically obtains when it polls the Apollo Uplink.
...
In this example, both the Products and Inventory subgraphs can resolve `Product.name`. This increases the likelihood that a client's query can be resolved by fewer total subgraphs.
...
> Note that to include a field in a `@provides` directive, that field must be marked as `@shareable` or `@external` in every subgraph that defines it.
