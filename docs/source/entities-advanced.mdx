...
Use the `@context` and `@fromContext` directives to set and get a _context_ through which types of a subgraph can share data. Contexts enable a subgraph to share data between types of a nested-type hieararchy, without overloading entity keys with extraneous fields and without breaking any separation of concerns between different subgraphs.  
