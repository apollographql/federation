...
* A **gateway** that composes the subgraphs into a **federated supergraph** and executes queries across multiple subgraphs
...
* **Subgraph schemas.** Each of your **subgraphs** has its own schema that represents the types and fields of your **supergraph** that it's responsible for resolving.
...
* **API schema.** This schema is like the supergraph schema, but it _omits_ federation-specific directives. This is the schema that your gateway exposes to your GraphQL API's consumers, who don't need to know any federation-specific details about your graph.
...
The gateway for a federated data graph provides its composed [supergraph schema](#supergraph-schema) to the `ApolloGateway` constructor:
