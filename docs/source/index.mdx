...
>
> Unchanged portions of the [Federation 1 docs](/federation/) docs are omitted for clarity.
...
* The gateway runs Apollo Server using special extensions from the [`@apollo/gateway`](./api/apollo-gateway) library.
    * Alternatively, the gateway can be an instance of [the Apollo Router](https://www.apollographql.com/docs/router/), a highly performant Rust-based graph router that is currently in public pre-alpha.
...
> Some federation-specific directives are omitted here for clarity. For details, see [Entities](./entities/).
