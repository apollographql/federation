...
> 📣 **Apollo Federation 2 is available in public alpha.** [View the Federation 2 docs.](https://www.apollographql.com/docs/federation/v2/)
...
Consider the `recentPurchases` field of the `User` type in the above schema. Even though this field is a member of the `User` type, a list of `Products` should probably be populated by the `products` subgraph, _not_ the `users` subgraph.
