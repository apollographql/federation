...
GraphQL developers can quickly encounter the infamous "N+1 query problem" with operations that return a list.
For example, take this `TopReviews` query:
...
If the `Query.topReviews` field returns ten reviews, then the executor resolves `Review.product` field ten times.
...
- Fetching the ten products in a single query is more efficient—for example, `SELECT * FROM products WHERE id IN (<product ids>)`.
...
Most subgraph implementations use _reference resolvers_ (sometimes known as _entity resolvers_) to handle the `Query._entities` field ergonomically.
A [reference resolver](./#2-define-a-reference-resolver) is passed a single key and returns the entity object corresponding to that key.
...
The router executes two queries on its subgraphs to resolve the above operation:

1. It queries the Reviews subgraph to fetch all fields except `Product.name` and `Product.imageURL`.
2. It queries the Products subgraph to fetch each product's `name` and `image.URL`.
...
A basic implementation of the `fetchProductByID` function might make a database call each time it's called.
...
This problem can cause performance problems or even enable denial-of-service attacks if not prevented.
...
Most subgraph implementations (including [`@apollo/subgraph`](/apollo-server/using-federation/api/apollo-subgraph/)) don't write the `Query._entities` resolver directly.
...
The solution—whether for federated or monolithic graphs—is the [dataloader](https://github.com/graphql/dataloader) pattern.
For example, in an Apollo Server implementation, this could look like the following:
