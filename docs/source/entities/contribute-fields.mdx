...
subtitle: Learn how individual subgraphs can contribute to and reference fields from an entity
description: Learn individual subgraphs can contribute to and reference fields from an entity in a federated GraphQL architecture using the @required and @external directives.
---
...
To override this default behavior, see [Resolving another subgraph's field](./resolving-another-subgraphs-field).

Each subgraph that contributes fields to an entity must [define a reference resolver](./#2-define-a-reference-resolver) for that entity.
