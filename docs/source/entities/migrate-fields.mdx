...
3. Publish the Billing subgraph's updated schema to GraphOS with [`rover subgraph publish`](/graphos/schema-development/pipeline/delivery/publish/#subgraph-schemas).
...
If you're using [manual composition](/graphos/schema-development/federated-schemas/composition#manually-with-the-rover-cli), each schema change requires redeploying your router.
