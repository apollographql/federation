...
Subgraph B could have added `Book.reviews` by [contributing the field](/graphos/schema-development/federated-schemas/entities/contribute-fields) directly to the entity as usual. However, what if we wanted to add `reviews` to a hundred different entity implementations of `Media`?
