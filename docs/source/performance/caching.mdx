...
> Using cache control with Apollo Federation requires at least v3.0.2 of Apollo Server in your subgraphs and gateway. Please note that Apollo Server's cache hints API has evolved as of v3, so be sure to review the [updated caching documentation](/apollo-server/performance/caching/).
