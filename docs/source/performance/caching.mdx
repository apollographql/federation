...
The subgraph will calculate and set the cache hint for the response that it sends to the gateway and the gateway will then calculate the cache hint for the overall response based on the most restrictive settings among all of the responses received from the subgraphs involved in query plan execution.

Cache hints may also be set [dynamically in subgraph resolvers](https://www.apollographql.com/docs/apollo-server/performance/caching/#in-your-resolvers-dynamic).
...
The missions subgraph, however, will not include a `cache-control` header in its response to the gateway, thus indicating that it is not cacheable, and in turn, making the entire response sent to the client not cacheable. At first glance, this behavior may seem unexpected because the `@cacheControl(maxAge: 10)` directive is applied to the `mission` root query field. But upon closer inspection, we see that this field returns the `Mission` type which contains a non-scalar `Astronaut` field and this type will have a [default `maxAge` of `0` applied](https://www.apollographql.com/docs/apollo-server/performance/caching/#default-maxage).
