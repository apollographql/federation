...
# This is an optional directive (see the glossary below)
...
The value returned for the `sdl` field should include all of this information, including directives (excessive whitespace can be removed).
...
These are the representation fields that the router automatically added to its Products query above.
