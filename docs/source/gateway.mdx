...
To update your version of the `@apollo/gateway` library, use the [`npm update`](https://docs.npmjs.com/cli/v7/commands/npm-update) command:

```bash
npm update @apollo/gateway
```

This updates the library to the latest version allowed by your `package.json` file. [Learn more about dependency constraints.](https://docs.npmjs.com/cli/v7/commands/npm-update#example)

To update to a particular version that _exceeds_ your depedency constraints, use `npm install` like so:
