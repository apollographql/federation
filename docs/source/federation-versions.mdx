...
[`@interfaceObject`](https://www.apollographql.com/docs/federation/federated-types/interfaces) a directive that allows you to abstract an interface in isolation by declaring it locally as an object type. This enables you to contribute fields without needing to redefine all the interface’s concrete implementation types.
...
The `@shareable` directive has been https://github.com/apollographql/federation/pull/2175. This allows you to use in a type definition and in an extension of that type definition.
...
indicates to composition that all uses
...
**Note:** make sure to include the `@composeDirective` directive in the `import` list of the `@link` directive that points to the spec that defines the directive.
