...
- To use a feature introduced in a particular federation version, make sure your subgraph schema's `@link` directive targets that version (or higher):
    ...
    Before you increment a subgraph's federation version, update your router and build pipeline. For details, see [Updating your graph safely](/graphos/reference/migration/updates).
...
Introduced. [Learn more](/graphos/federation/federated-schemas/federated-directives#cost).
...
Introduced. [Learn more](/graphos/federation/federated-schemas/federated-directives#listsize).
...
Introduced. [Learn more.](/graphos/router-configuration/security/authorization)
...
- Use of GraphQL subscriptions with a federated graph requires a compatible version of the GraphOS Router. [See details.](/graphos/router-configuration/operations/subscriptions/#prerequisites)
