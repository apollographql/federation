...
```graphql
directive @connect(source: String, http: ConnectHTTP!, selection: JSONSelection!, entity: Boolean) on repeatable on FIELD_DEFINITION
```
...
```graphql
directive @listSize(name: String!, http: SourceHTTP!) repeatable on SCHEMA
```
