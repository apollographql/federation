...
For details, see [Federated ownership directives](../federated-types/ownership-directives/).
...
You'll be able to override this enforcement with the [`@shared` directive](../federated-types/ownership-directives/#shared):
...
Now, either subgraph can resolve `Query.books`. The return types of these shared fields can even vary in the same ways as [other shared types](./new-in-federation-2/#valid-field-differences-between-subgraphs).
...
> You can disable this "single source of truth" behavior with the [`@shared` directive](../federated-types/ownership-directives/#shared).
