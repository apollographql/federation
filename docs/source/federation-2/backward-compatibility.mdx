...
A subgraph can mark an entity field with the [`@provides` directive](https://www.apollographql.com/docs/federation/federated-types/federated-directives/#provides) to indicate that it resolve entity fields normally marked as `@external` if they are instead resolved through this particular path:
...
In this example, if we traverse through the `outOfStockProducts` Query, Subgraph A can resolve the `Product.info.name` field, otherwise any other path will result in another subgraph call.

Federation 1 _incorrectly_ allows a `@provides` directive such as the following:
...
This `@provides` directives should _break_ composition here because it does not specify which subfields of `ProductInfo` that it can resolve.
