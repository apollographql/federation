...
Phase 1 requires very few changes that are straightforward to move _back_ to Federation 1. However, Phase 2 requires schema changes that _remove_ backward compatibility with Federation 1. We strongly recommend performing Phase 2 with a test instance of your graph first.
...
This definition identifies a schema as a true Federation 2 schema. Without this definition, Federation 2 assumes a subgraph schema is a Federation 1 schema and uses intelligent defaults to support backward compatibility.
...
Federation 2 makes subtle but powerful changes to [entities](../entities/) that require corresponding changes to their definitions in your subgraph schemas.
...
In Federation 2, entities no longer have an originating subgraph. Instead, each subgraph can define the entity and contribute fields to it:
...
* Both subgraphs can resolve `Product.id`, but it isn't marked as `@shareable`.
...
However, if a subgraph `@provides` a particular field, that field must be marked as `@shareable` in the subgraphs where it's _always_ resolvable:
