...
Update your gateway's `@apollo/gateway` library to version `2.0.0-preview.0` or later. This enables your gateway to interact with supergraph schemas composed with Federation 2 logic.
...
1. Install the latest version of Rover (must be `0.4.4` or later) using the appropriate command for your system:
...
In Federation 2, the Reviews subgraph above does _not_ need to provide a `@key` for the `Product` entity, because it doesn't contribute any fields to `Product`. Instead, it only uses `Product` as a return type for `Review.product`.

Therefore, the Reviews subgraph can change its `Product` definition to the following:
...
Note the lack of a `@key` on `Product`, _and_ the addition of the new `@shareable` directive to `Product.id`. The Products subgraph _doesn't_ need to apply `@shareable`, because `id` is a `@key` field in that subgraph (`@key` fields are `@shareable` by default).
