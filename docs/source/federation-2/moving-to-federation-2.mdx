...
- The Inventory subgraph no longer marks the `Product.id` field as `@external`.
    - The `@external` directive is no longer required for `@key` fields, but it is still required for [`@requires`](/graphos/schema-development/federated-schemas/entities/contribute-fields#contributing-computed-entity-fields) and [`@provides`](../entities/resolve-another-subgraphs-fields#using-provides).
