...
2. Install the Federation 2 plugin for Rover using the appropriate command for your system:
    ...
    ```bash title="MacOS / Unix-like"
    curl -sSL https://rover.apollo.dev/plugins/rover-fed2/nix/latest | sh
    ```
...
> **Note:** Most subgraph schemas use the `@key` and `@shareable` directives, so we've included them in the snippet above. Depending on your schema, you might need to add other federated directives to the `import` array, such as:
>
> * `@external`
> * `@provides`
> * `@requires`
>
> See also [Federation-specific GraphQL directives](../federated-types/federated-directives/).
