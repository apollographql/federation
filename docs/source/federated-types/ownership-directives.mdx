...
In a federated graph, types and fields can cross subgraph boundaries, appearing in more than one subgraph schema:
...
In this example, both Subgraph A and Subgraph B can resolve `Query.books`. They can even define different return types if those return types are [compatible](./value-types/#varying-field-return-types).
