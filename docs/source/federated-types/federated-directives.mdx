...
Indicates that a non-`@shareable` entity field is now resolved by _this_ subgraph instead of the _one other_ subgraph where it's currently defined.
...
You use `@override` to migrate an entity field from one subgraph to another. In the example above, we're migrating the `Product.inStock` field from the Products subgraph to the Inventory subgraph. The composed supergraph schema indicates that `Product.inStock` is resolved _only_ by the Inventory subgraph, even though the Products subgraph also defines the field.
