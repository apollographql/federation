...
The `@context` directive defines a named context from which any field of the annotated type can be passed to a receiver of the context (a field annotated with [`@fromContext`](#fromcontext)). 
...
Each `@context` must be defined with a name that's unique within the supergraph.
...
The `@fromContext` directive sets the context from which to get the value of the annotated field.
...
A `@fromContext` directive must be used as an argument on a field. Its field value, `FieldValue`, must contain the name of a valid context—one defined by `@context`—and a selection of a field from the context's type.

The selection syntax for `@fromContext` used in its `FieldValue` is GraphQL field selection syntax with some added rules: 
...
- All fields referenced in the `FieldValue` must be expressed within the current subgraph. If necessary, the fields must be annotated with `@external`.
