...
> **⚠️ This directive is available in Apollo Federation 2.3 and later.**
...
You can apply `@override` to [entity](../entities/) fields, along with fields of the root operation types (such as `Query` and `Mutation`).
...
* **If you perform composition with managed federation,** this corresponds to the subgraph name you've published (or will publish) to GraphOS.
* **If you perform composition with the Rover CLI,** this corresponds to the subgraph name in the YAML config file you provide to `rover supergraph compose`.

You _can_ provide the name of a subgraph that you haven't yet added to your supergraph. When you later add that subgraph, composition honors the directive.
...
> **⚠️ This directive is available in Apollo Federation 2.1 and later.**
