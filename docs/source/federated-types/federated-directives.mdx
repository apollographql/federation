...
Indicates to composition that the target element is restricted based on authorization policies that are evaluated in a Rhai script or coprocessor. Refer to the [rotuer article](/router/configuration/authorization#policy) for additional details.
