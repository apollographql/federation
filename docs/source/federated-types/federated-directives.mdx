...
> ⚠️ **This is an [Enterprise feature](/router/enterprise-features) of the Apollo Router.** It requires an organization with a [GraphOS Enterprise plan](https://www.apollographql.com/pricing/).</br>
>
...
Indicates to composition that target element is accessible only to the authenticated supergraph users. For more granular access control see [`@requiresScope`](#requiresScope) directive below. See [JWT Authentication in the Apollo Router](/router/configuration/authn-jwt) for additional details.
...
Indicates to composition that target element is accessible only to the authenticated supergraph users with appropriate JWT scope. See [JWT Authentication in the Apollo Router](/router/configuration/authn-jwt) for additional details.
...
**Required.** List of JWT scopes that have to be granted to the user in order to access the underlying element data.
