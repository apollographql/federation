...
> ⚠️ **This directive's feature is available in Apollo Federation TBD and later.**
...
Rolling out any change to a production subgraph, including adding `@override` fields, risks degrading the performance of your graph. Rerouting all traffic from one subgraph to another _all at once_ could overload the overriding subgraph.

The **progressive `@override`** feature enables the gradual, progressive deployment of a subgraph with an `@override` field. With progessive `@override` enabled in the Apollo Router, a subgraph developer can apply a label to an `@override` field that sets the percentage of traffic for the field that should be resolved by the overriding subgraph, with the rest still resolved by the overridden subgraph. The subgraph developer can then monitor the performance of the subgraphs in Studio, resolve any issues, and iteratively and progressively increase the percentage until all traffic can be handled by the overriding subgraph.

To configure the Apollo Router for progressive `@override`, see [TBD](/router/configuration/overview).
...
**Optional.** The percentage of traffic for this field (randomly chosen from a uniform distribution) that is resolved by this subgraph. The remaining percentage is resolved by the other ([from](#from)) subgraph.
