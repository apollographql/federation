...
To use federated directives in a Federation 2 subgraph schema, that schema must include an `extend schema` definition with the following format:
...
The `import` array must include whichever federated directives your subgraph schema uses. The example above imports the [`@key`](#key) and [`@shareable`](#shareable) directives (which are used most commonly).
...
Indicates that a particular subgraph can resolve particular object fields, but _only_ at a particular query path.
...
**Required.** A comma-separated list of object field names that the subgraph can resolve at this query path.
...
Indicates that a particular object field's resolver requires the values of _other_ object fields that are resolved by _other_ subgraphs.
