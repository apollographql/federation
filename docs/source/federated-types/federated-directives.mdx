...
Each `@context` must be defined with a name that's unique within the subgraph. 

Each `@context` can be applied to multiple places within a subgraph. For example:

```graphql
type A @key(fields: "id") @context(name: "ctxt") {
  id: ID!
  prop: String!
}

type B @key(fields: "id") @context(name: "ctxt") {
  id: ID!
  prop: String!
}
...
type U @key(fields: "id") {
  id: ID!
  field (arg: String @fromContext(field: "$ctxt { prop }")): String!
}
```
...
- The argument must be nullable. Because validation is currently done at the subgraph level, the referenced field may become nullable when merging subgraphs, such as when the field is nullable in one subgraph but not in another.
