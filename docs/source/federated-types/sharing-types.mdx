...
To use it in a subgraph schema, you first need to add the following snippet to that schema to [opt in to Federation 2](../federation-2/moving-to-federation-2/#opt-in-to-federation-2):
...
Applying the `@shareable` directive to a type is equivalent to applying it to every _field_ of that type. You can also share individual fields, like so:
