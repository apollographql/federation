...
By default in Federation 2 schemas, a single object field _can't_ be defined in more than one subgraph schema.

Take our `Position` example; let's say you'd like to define the `Position` type in two subgraphs that each can resolve for that type:
...
Applying the `@shareable` directive to an object type shares that type's fields across multiple subgraphs:
