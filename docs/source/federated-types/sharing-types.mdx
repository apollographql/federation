...
Compositional logic merges these definitions differently in your supergraph schema depending on whether they are used as an input or output type. Output types are merged by "union", and input types are merged by "intersection". If the above are output types, the composed schema looks like this:
...
Whereas if they are input types, this would be the result:
...
For output types, the supergraph schema lists all values that may be returned, but for input types we restrict values that are allowed in the shared type so that we can ensure all subgraphs can process the value provided.
