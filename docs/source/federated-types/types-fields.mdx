...
* A field's return type can vary in nullability and count (list vs. single value).
* A field's return type _can't_ vary in its core type (e.g., `String` versus `Int`).
...
Composition can use coercion to handle differences in field **nullability** and/or **count** (list vs. single value). Look at these varying definitions of a `Document` object type:
...
In Federation 2, enum and union value types can have _completely different_ definitions between subgraphs:
