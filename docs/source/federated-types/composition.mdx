...
> For examples of _valid_ inconsistencies in field return types, see [Varying shared field return types](./sharing-types/#varying-shared-fields).
...
* Multiple subgraphs can not [redefine the same field on an Object type](./sharing-types/#explicit-and-implicit-type-sharing) _unless_ that field is shared.
* A shared field must have both a compatible return type _and_ compatible argument types across each defining subgraph.
    * For examples of compatible and incompatible field return types, see [Varying shared fields](./sharing-types/#varying-shared-fields).
...
`Position.z` is an example of a shared field that is _not_ always resolvable. So now, how do we make sure that such a field _is_ always resolvable?
...
If every subgraph that defines a type _could_ resolve every shared field of that type without introducing complexity, a straightforward solution is to define and resolve all fields in all of those subgraphs:
