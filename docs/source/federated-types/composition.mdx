...
> For examples of _valid_ inconsistencies in field return types, see [Varying field return types](./value-types/#varying-field-return-types).
...
* If multiple subgraphs define the same field, that field must be [shared](./sharing-types).
* A shared field must have both a compatible return type _and_ compatible argument types across each defining subgraph.
    * For examples of compatible and incompatible field return types, see [Varying field return types](./sharing-types/#varying-field-return-types).
