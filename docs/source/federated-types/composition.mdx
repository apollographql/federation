...
During composition, two strategies are used under the hood to merge different schema types: _union_ and _intersection_.
...
When the above subgraphs are composed, the supergraph schema will merge corresponding types using the union strategy, resulting in the following supergraph schema:
...
Composition uses the intersection strategy to merge both input types and field arguments. The intersection strategy incorporates only the fields or arguments that every subgraph has _in common_ into the supergraph schema. 
...
The intersection merging strategy only merges the matching fields between input types, any field that isn't in every subgraph gets left out of the supergraph schema. The same is true for field arguments:
...
type Library {
  # field arguments are composed using intersection 
  findBook(title: String): Book
}
```

> When composing input types and field arguments for multiple subgraphs, ensure that every non-nullable field and field argument is consistent in every subgraph. Otherwise, if _any_ input type or field argument is missing a non-nullable field in one subgraph, your supergraph schema won't contain that field.  

The intersection strategy ensures your gateway knows that any subgraphs can resolve for the field arguments in an incoming GraphQL operation. 

### Merging enums

Enums are the only type that can be merged 

Enums are the only type that can be merged by using either the [union](#merging-objects-unions-and-interfaces) or [intersection](#merging-input-types-and-field-arguments) strategies.  The merging strategy 

For example, if you define an enum in multiple subgraphs, like so:
...
Composition will use the [union merging strategy](#merging-objects-unions-and-interfaces) to incorporate all of your enum's fields, resulting in the following supergraph schema:
...
However, this changes if you include your enum in either an input type or as a field argument.

Going back to our previous example, let's say we'd like to use our enum as a field argument, like so:
...
Because our `enum` is now being used within an `input` type, this changes the  merging strategy that composition uses. 

When an enum is includedThis means that now only the _common_ properties will be merged into our supergraph schema, resulting in a supergraph schema like this:
...
If you'd like all of our `enum`'s' values to all be present in our supergraph schema you should ensure your `enums` have the same values across all subgraphs.
