...
> For examples of _valid_ inconsistencies in field return types, see [Varying shared field return types](./sharing-types/#varying-shared-field-return-types).
...
* Multiple subgraphs can not [redefine the same field on the same type](./sharing-types/#sharing-in-composition) unless that field is shared.
* A shared field must have both a compatible return type _and_ compatible argument types across each defining subgraph.
    * For examples of compatible and incompatible field return types, see [Varying shared field return types](./sharing-types/#varying-shared-fields-across-subgraphs).
...
## Fields not being shared leading to unresolvable fields

If you define a type across multiple subgraphs with the same fields you recieve and error when you attempt composition. 
...
To successfully compose, you must mark the fields you'd like to share across subgraphs withe the `@shareable` directive, like so:
