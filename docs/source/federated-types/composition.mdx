...
The resulting supergraph schema changes depending on which merging strategy composition uses. 
...
When the above subgraphs are composed, the composition process merges the corresponding types using the union strategy resulting in the following supergraph schema:
...
### Merging inputs and field arguments
...
Enums are the only type that composition can merge using either union or intersection. Composition's merging strategy for an enum is dictated by how your subgraphs use that enum. 
...
Composition uses the [union merging strategy](#merging-types-during-composition) to incorporate all of your enum's fields, resulting in the following supergraph schema:
...
Let's say we want to use our previous `enum` as an argument for a new field, like so:
...
The composition process only incorporates the _shared_ values between our two `enum` type definitions in the newly composed supergraph schema:
...
If we want all of our enum's possible values to be present in the supergraph schema, we can ensure our `enum` has the same values in every subgraph:
...
Now, even though composition is merging via intersection, the resulting supergraph schema has all of our `enum`'s values:
...
By ensuring all of our subgraphs define an enum the same way, the composition process includes all of our enum's values:
