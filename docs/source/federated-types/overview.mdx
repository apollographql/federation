...
As these schemas show, a subgraph can define and even extend types that are defined in _other_ subgraphs. For example, the Reviews subgraph extends the `User` type (from the Users subgraph) and adds a `reviews` field to it.
