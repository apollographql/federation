...
**Any given instance of an entity must be uniquely identifiable by its `@key` fields.** This is what enables your gateway to associate data from _different_ subgraphs with the _same_ entity instance.
...
With this definition in place, the Reviews subgraph can then add a field of type `Product` to its `Review` type, like so:
...
> Notice that the stub doesn't even need a `@key` directive! That's because the Reviews subgraph isn't adding any of its _own_ fields to `Product`. We'll see that later in [Extending entities](#extending-entities).
...
This resolver's return value is a **representation** of a `Product` entity. A representation always consists of:
...
A subgraph can add its own fields to an entity that's already defined in other subgraphs. This is called **extending** the entity:
...
When a subgraph extends an entity, no _other_ subgraph knows about the added fields—only the gateway does thanks to the composed supergraph schema.
...
Let's say we want to be able to query for the `inStock` status of a product. That information lives in the Inventory subgraph, so we'll extend the entity there:
