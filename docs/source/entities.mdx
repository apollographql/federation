...
**Every definition of an entity across subgraphs must be uniquely identifiable by its `@key` fields.** This is what enables your router to associate field data from _different_ subgraphs with the _same_ entity instance. i.e. If one subgraph marks the key as `id` and another uses `name`, the router will not be able to join those instances together.
...
For more advanced key options, like how to define multiple keys or compound keys, [see advanced options for federation entities.](./entities-advanced)
