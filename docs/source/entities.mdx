...
> ⚠️ **This section describes how to create reference resolvers in Apollo Server.** If you're using another [subgraph-compatible library](./other-servers/), see its documentation for creating reference resolvers.

For the `Product` entity defined [above](#define-a-key), the reference resolver might look like this:
...
* A reference resolver is responsible for returning all of the entity fields that this subgraph defines.
    * In this example, the hypothetical `fetchProductByID` field fetches a particular `Product`'s field data based on its `id`.

_Every_ subgraph that contributes at least one field to an entity must define a reference resolver for that entity. If you're using Apollo Server for your subgraph, it defines a [default reference resolver](#default-reference-resolver) for any entities you _don't_ define one for.
...
## Referencing an entity _without_ contributing fields

Your subgraphs can use an entity as a field's return type _without_ contributing any fields to that entity. This requires much less code than the steps in [Defining an entity](#defining-an-entity).
...
As you can see, this stub definition includes _only_ the `@key` fields of `Product` (just `id` in this case). It also marks the `id` field as `@shareable`, which is required because the field is resolved by multiple subgraphs.

> The `@shareable` directive isn't required for `id` in the Products subgraph because `id` is included in the `@key` directive there. Fields listed in a `@key` directive are automatically `@shareable`.
