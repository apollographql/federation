...
1. In the Billing subgraph's schema, define the `Bill` entity just as it's defined in the Payments subgraph (do _not_ extend it):
    ...
    * Note that if you perform composition at this point, it produces an error because the `Bill` entity can't originate in more than one subgraph. That's okay! We'll resolve this error in a couple of steps.
...
2. Publish the updated Billing subgraph schema to Apollo.
    ...
    * Note that you're publishing a composition error, because the `Bill` entity now originates in more than one subgraph. That's okay! Your gateway continues to use your most recent _valid_ supergraph schema.
