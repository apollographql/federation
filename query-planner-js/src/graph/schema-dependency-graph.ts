import { FieldNode, GraphQLField, GraphQLObjectType, GraphQLSchema, GraphQLNamedType  } from "graphql";
...
interface OperationSchemaNode {fieldDef: GraphQLField<any, any>, fieldNode?: GraphQLField<any, any>}

export class SchemaDependencyGraph {

    public dependenciesAdjacencySet: Map<GraphQLField<any, any>, Set<GraphQLField<any, any>>> = new Map<GraphQLField<any, any>, Set<GraphQLField<any, any>>>();
        ...
        types.forEach(type => {
            ...
            const typeNameDefMap: Map<string, GraphQLField<any, any>> = new Map(fieldDefs.map(fieldDef => [fieldDef.name, fieldDef]));
            fieldDefs.forEach(fieldDef => {
                ...
                schemaGraph.addRequiresDependencies(dependencyNodes as FieldNode[], fieldDef, typeNameDefMap);
            })
        })
        return schemaGraph;
    }

    public getDependencySubgraph(fields: FieldSet): { subgraph: SchemaDependencyGraph, paths: Map<GraphQLField<any, any>, string[]> } {
        ...
        depthFirstSearch({
            ...
            visit: (node: any) => {
                ...
                const newPath = addPath(path, getResponseName(fieldNode), fieldDef.type) as string[];
                paths.set(fieldDef, newPath)
            },
            getChildren: (node: any) => {
                // traverse operation structure edges
                const structuralChildren: OperationSchemaNode[] = node.fieldNode?.selectionSet?.selections.map((fieldNode: FieldNode) => {
                    const fieldDefs = node.fieldDef.type.getFields()
                    const fieldDef = Object.values(fieldDefs).find((fieldDef: any) => fieldDef.name === fieldNode.name.value)
                    return {fieldDef, fieldNode}
                }) || []

                // traverse schema dependency edges
                const dependencyChildren: OperationSchemaNode[] = this.getDependencies(node.fieldDef)
                    ?.filter((dep: GraphQLField<any, any> | undefined): dep is GraphQLField<any, any> => {
    ...
    public getDependencies(node: GraphQLField<any, any>): GraphQLField<any, any>[] {
    ...
    private addKeyDependencies(type: GraphQLNamedType, fieldDef: any, typeNameDefMap: Map<string, GraphQLField<any, any>>) {
        const keyFields = (type.extensions?.federation as FederationEntityTypeMetadata)?.keys;
        const fieldGraphName = fieldDef.extensions?.federation?.graphName;

        const fieldKeyNodes: FieldNode[] = (keyFields && fieldGraphName) ? keyFields
            .get(fieldGraphName)?.flat()
            .filter((fieldNodeOrFragment: any) => fieldNodeOrFragment['name']) as FieldNode[] : []
    ...
    private addRequiresDependencies(dependencyNodes: FieldNode[], fieldDef: GraphQLField<any, any>, nameDefMap: Map<string, GraphQLField<any, any>>) {
        const dependencyDefs: GraphQLField<any, any>[] = dependencyNodes
            .map((dependencyNode: FieldNode) => nameDefMap.get((dependencyNode as FieldNode).name.value))
            .filter((dependencyDef: GraphQLField<any, any> | undefined): dependencyDef is GraphQLField<any, any> => !!dependencyDef)

        this.addDependencies(fieldDef, dependencyDefs)
    }

    private addDependencies(node: GraphQLField<any, any>, dependencies: GraphQLField<any, any>[]): void {
        const existingDeps = this.dependenciesAdjacencySet.get(node) || []
        this.dependenciesAdjacencySet.set(node, new Set([...existingDeps, ...dependencies]))
    }
