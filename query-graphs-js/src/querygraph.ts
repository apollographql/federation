  ...
  addAdditionalAbstractTypeEdges() {
    ...
    for (let i = 0; i < abstractTypesWithTheirRuntimeTypes.length - 1; i++) {
      ...
      for (let j = i; j < abstractTypesWithTheirRuntimeTypes.length; j++) {
        ...
        // We ignore the pair of abstract types in case the supergraph contains
        // type that unknown to the given runtime
        if (t2RuntimesGlobal.length > t2Runtimes.length) {
          continue;
        }

        if (t1RuntimesGlobal.length > t1Runtimes.length) {
          continue;
        }
