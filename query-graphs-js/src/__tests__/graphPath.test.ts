...
describe("advanceSimultaneousPathsWithOperation", () => {
  test("do not use key `x` to fetch `x`", () => {
    ...
    const allAfterT = advanceSimultaneousPathsWithOperation(supergraph, initial, field(api, "Query.t"), new Set());
    ...
    const allForId = advanceSimultaneousPathsWithOperation(supergraph, afterT, field(api, "T.id"), new Set());
  ...
  test("do not use key containing `x` to fetch `x`", () => {
    ...
    const allForId = advanceSimultaneousPathsWithOperation(supergraph, afterT, field(api, "T.id1"), new Set());
    assert(allForId, 'Should have advanced correctly');
