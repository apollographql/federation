...
- Send error reports to a configurable `APOLLO_OUT_OF_BAND_REPORTER_ENDPOINT=https://uplink.api.apollographql.com/monitoring` owned by Apollo. Using the url `https://uplink.api.apollographql.com/monitoring` is recommended unless you have a custom configuration. Reports will only be sent if the endpoint value is set in `env`.  [PR #777](https://github.com/apollographql/federation/pull/777/files)
